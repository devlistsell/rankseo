    ...
    >
        @foreach ($plans as $key => $plan)
            <tr class="position-relative">
                    ...
                    <div class="text-nowrap d-flex align-items-canter">
                        @if (!$plan->visibility)
                            <a
                                ...
                                link-method="POST"
                                href="{{ action('Admin\SendingCreditPlanController@visibilityOn', $plan->uid) }}"
                        ...
                        @else
                            <a
                                ...
                                link-method="POST"
                                href="{{ action('Admin\SendingCreditPlanController@visibilityOff', $plan->uid) }}"
                ...
                <td>
                    <span class="no-margin stat-num">{{ Auth::user()->customer->formatDateTime($plan->created_at, 'datetime_full') }}</span>
                    <br>
                ...
                <td>
                    ...
                    <span class="text-muted">{{ trans('messages.sending_credit_plan.credits') }}</span>
                </td>
                <td class="text-end text-nowrap pe-0" width="5%">
                    <div class="list-actions">
                        <a href="{{ action('Admin\SendingCreditPlanController@edit', $plan->uid) }}" role="button" class="btn btn-secondary btn-icon"> <span class="material-symbols-rounded">edit</span> {{ trans('messages.edit') }}</a>
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu dropdown-menu-end">
                                <li>
                                    <a class="dropdown-item list-action-single"
                                        link-method="POST"
                                        link-confirm-url="{{ action('Admin\SendingCreditPlanController@deleteConfirm', ['uids' => $plan->uid]) }}"
                                        href="{{ action('Admin\SendingCreditPlanController@delete', $plan->uid) }}"
                                        title="{{ trans('messages.sending_credit_plan.delete') }}" class=""
                                    >
                                        <span class="material-symbols-rounded">delete</span> {{ trans('messages.sending_credit_plan.delete') }}
                                    </a>
                                </li>
                            </ul>
                        </div>
